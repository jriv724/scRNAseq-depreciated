...
function (raw_counts_matrix, gene_order_file, annotations_file, 
    ref_group_names, delim = "\t", max_cells_per_group = NULL, 
    min_max_counts_per_cell = c(100, +Inf), chr_exclude = c("chrX", 
        "chrY", "chrM")) 
{
    if (Reduce("|", is(raw_counts_matrix) == "character")) {
        flog.info(sprintf("Parsing matrix: %s", raw_counts_matrix))
        if (substr(raw_counts_matrix, nchar(raw_counts_matrix) - 
            2, nchar(raw_counts_matrix)) == ".gz") {
            raw.data <- read.table(connection <- gzfile(raw_counts_matrix, 
                "rt"), sep = delim, header = TRUE, row.names = 1, 
                check.names = FALSE)
            close(connection)
            raw.data <- as.matrix(raw.data)
        }
        else if (substr(raw_counts_matrix, nchar(raw_counts_matrix) - 
            3, nchar(raw_counts_matrix)) == ".rds") {
            raw.data <- readRDS(raw_counts_matrix)
        }
        else {
            raw.data <- read.table(raw_counts_matrix, sep = delim, 
                header = TRUE, row.names = 1, check.names = FALSE)
            raw.data <- as.matrix(raw.data)
        }
    }
    else if (Reduce("|", is(raw_counts_matrix) %in% c("dgCMatrix", 
        "matrix"))) {
    ...
    if (Reduce("|", is(gene_order_file) == "character")) {
        ...
        gene_order <- read.table(gene_order_file, header = FALSE, 
            row.names = 1, sep = "\t", check.names = FALSE)
    }
    else if (Reduce("|", is(gene_order_file) %in% c("dgCMatrix", 
        "matrix", "data.frame"))) {
    ...
    if (!is.null(chr_exclude) && any(which(gene_order$chr %in% 
        chr_exclude))) {
        gene_order = gene_order[-which(gene_order$chr %in% chr_exclude), 
            ]
    }
    if (Reduce("|", is(annotations_file) == "character")) {
        flog.info(sprintf("Parsing cell annotations file: %s", 
            annotations_file))
        input_classifications <- read.table(annotations_file, 
            header = FALSE, row.names = 1, sep = delim, stringsAsFactors = FALSE, 
            colClasses = c("character", "character"))
    }
    else if (Reduce("|", is(annotations_file) %in% c("dgCMatrix", 
        "matrix", "data.frame"))) {
    ...
    if (!all(rownames(input_classifications) %in% colnames(raw.data))) {
        missing_cells <- rownames(input_classifications)[!(rownames(input_classifications) %in% 
            colnames(raw.data))]
        error_message <- paste("Please make sure that all the annotated cell ", 
            "names match a sample in your data matrix. ", "Attention to: ", 
    ...
    if (num_genes_removed > 0) {
        flog.info(paste("num genes removed taking into account provided gene ordering list: ", 
            num_genes_removed, " = ", num_genes_removed/dim(raw.data)[1] * 
                100, "% removed.", sep = ""))
    }
    ...
    if (is.null(raw.data)) {
        error_message <- paste("None of the genes in the expression data", 
            "matched the genes in the reference genomic", "position file. Analysis Stopped.")
        stop(error_message)
    }
    if (is.null(min_max_counts_per_cell)) {
        min_max_counts_per_cell = c(1, +Inf)
    }
    min_counts_per_cell = max(1, min_max_counts_per_cell[1])
    ...
    flog.info(sprintf("-filtering out cells < %g or > %g, removing %g %% of cells", 
        min_counts_per_cell, max_counts_per_cell, n_to_remove/n_orig_cells * 
            100))
    raw.data <- raw.data[, cells.keep]
    input_classifications <- input_classifications[rownames(input_classifications) %in% 
        colnames(raw.data), , drop = FALSE]
    orig_ref_group_names = ref_group_names
    ref_group_names <- ref_group_names[ref_group_names %in% unique(input_classifications[, 
        1])]
    ...
    if (!is.null(max_cells_per_group)) {
        grps = split(input_classifications, input_classifications[, 
            1])
        newdf = NULL
        for (grp in names(grps)) {
            ...
            if (dim(df)[1] > max_cells_per_group) {
                ...
                grps[[grp]] = df[sample(seq_len(dim(df)[1]), 
                  max_cells_per_group), , drop = FALSE]
            }
        }
        input_classifications = data.frame(Reduce(rbind, grps))
    }
    raw.data <- raw.data[, colnames(raw.data) %in% rownames(input_classifications)]
    input_classifications <- input_classifications[order(match(row.names(input_classifications), 
        colnames(raw.data))), , drop = FALSE]
    ...
    for (name_group in ref_group_names) {
        ...
        if (length(cell_indices) == 0) {
            stop(sprintf("Error, not identifying cells with classification %s", 
                name_group))
        }
    ...
    object <- new(Class = "infercnv", expr.data = raw.data, count.data = raw.data, 
        gene_order = input_gene_order, reference_grouped_cell_indices = ref_group_cell_indices, 
        observation_grouped_cell_indices = obs_group_cell_indices, 
        tumor_subclusters = NULL, options = list(chr_exclude = chr_exclude, 
            max_cells_per_group = max_cells_per_group, min_max_counts_per_cell = min_max_counts_per_cell, 
            counts_md5 = digest(raw.data)), .hspike = NULL)
    validate_infercnv_obj(object)
    return(object)
}
<bytecode: 0x7fe15fa45e78>
